const processStreamingResponse = async (session, text) => {
  ...
  if (says === 0) {
    ...
    if (-1 !== pos) {
      ...
      session.say({ text: firstSentence }).send({ execImmediate: true });
      session.locals.says++;
  ...
  if (says > 0) {
    ...
    if (-1 !== pos) {
      ...
      session.say({ text: paragraph }).send({ execImmediate: false });
      session.locals.says++;
...
const streamingResponseComplete = async (session) => {
  ...
  if (unsent) {
    ...
    session.say({ text: unsent }).send({ execImmediate: false });
  }
